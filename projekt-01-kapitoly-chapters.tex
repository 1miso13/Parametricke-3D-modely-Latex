...
\section{Boolovské operácie nad objemovými telesami}
Vo výsledku by sa malo dať vytvárať nad objektami boolovské operácie ako zjednotenie (union), prienik (intersection) a rozdiel medzi množinami (minus). Keďže je táto operácia náročná na spracovanie, rozhodol som sa využiť niektorú z voľne dostupných knižníc. Našiel som niekoľko dostupných knižníc, ktoré by sa dali použiť. Jednou z nich je \texttt{libigl}\cite{libigl} ale na integráciu do aplikácie zatiaľ nie sú dostatočne spracované objemové objekty.
...
Obľúbená je hlavne pre svoju jednoduchosť. Tvorba booluvských operácii nad mesh stačí zavolať funkciu: 
