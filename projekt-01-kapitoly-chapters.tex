...
V tejto časti je opísaný aktuálny stav práce, návrh možností previazania objektov v parametrickom modely a implementácia knižnice určenej na tvorbu parametrických 3D modelov, kde sú tieto možnosti previazania implementované.  

\section{Koncept práce}

Táto knižnica musí byť intuitívna a ľahko použiteľná pre používateľa.

Vlastnosti ktoré by mala táto knižnica obsahovať:
\begin{itemize}
\item Implementácia knižnice:
...
Jednotlivé operácie sa oddeľujú bodkočiarkou (\texttt{;}). 
Táto textová podoba geometrických operácií sa používa pri vytváraní modelu a aj pre uloženie do súboru a jeho neskoršie načítanie.
...
Vytvoril som aj jednoduché grafické rozhranie, ktoré návrhárovi pomáha, pri vytváraní parametrického modelu a uľahčuje prácu s geometrickými operáciami, a zároveň pri tom zobrazuje aj internú štruktúru modelu, teda jednotlivé operácie, objekty a parametre a zároveň tento model aj vizualizuje.
Táto aplikácia je vytvorená pomocou knižnice QT a je zobrazená na obrázku \ref{fig:GraphicEditor}.


\begin{figure}[hbt]
	\centering
	\includegraphics[width=1\textwidth]{obrazky-figures/placeholder.pdf}
	\caption{Grafický editor}
...
Počet geometrických operácii: 46
\begin{itemize}
    \item 15 bodových
    \item 13 čiarových
    \item 10 plošných
